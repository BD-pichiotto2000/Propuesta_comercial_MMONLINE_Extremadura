# Tabla de coordenadas correctas - clave: "Prov|Municipio" (sin acentos en provincia)
$coords = @{}

function Set-C($mun, $lat, $lon) {
    $coords["Caceres|$mun"] = @{lat=$lat; lon=$lon}
}
function Set-B($mun, $lat, $lon) {
    $coords["Badajoz|$mun"] = @{lat=$lat; lon=$lon}
}
...
Set-B 'Entrin Bajo' 38.720 -6.956
...
Set-C 'Valdehuнcar' 39.719 -5.507
...
# Funcion para quitar acentos y normalizar
function Strip-Accents($s) {
...
# Regex para cada entrada del array municipios
$pattern = "(\{prov:'([^']+)',\s*mun:'([^']+)',\s*cp:'[^']+',\s*dist:\d+,\s*zona:'[^']+',\s*lat:)([\d\.\-]+)(,\s*lon:)([\d\.\-]+)(,)"
$ms = [regex]::Matches($html, $pattern)

foreach ($m in $ms) {
    $provRaw = $m.Groups[2].Value
    $munRaw  = $m.Groups[3].Value
    $latOld  = [double]$m.Groups[4].Value
    $lonOld  = [double]$m.Groups[6].Value

    # Normalizar provincia y municipio (quitar acentos)
    $prov = Strip-Accents $provRaw
    $mun  = Strip-Accents $munRaw

    $key = "$prov|$mun"
    $c = $null
    if ($coords.ContainsKey($key)) { $c = $coords[$key] }
    ...
    if ($c -ne $null) {
        ...
        if ($diffLat -gt 0.05 -or $diffLon -gt 0.05) {
            $viejo = $m.Value
            $nuevo = "$($m.Groups[1].Value)$latNew$($m.Groups[5].Value)$lonNew$($m.Groups[7].Value)"
            $html = $html.Replace($viejo, $nuevo)
            $actualizados++
            Write-Host "  $provRaw | $munRaw : ($latOld,$lonOld) -> ($latNew,$lonNew)"
        }
    }
}
