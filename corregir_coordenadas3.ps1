...
# Usar regex con comillas simples escapadas como \x27
$pat = [regex]'(\{prov:''([^'']+)'',\s*mun:''([^'']+)'',\s*cp:''[^'']*'',\s*dist:\d+,\s*zona:''[^'']*'',\s*lat:)([\d.\-]+)(,\s*lon:)([\d.\-]+)(,)'
$ms = $pat.Matches($html)

foreach ($m in $ms) {
    $provRaw = $m.Groups[2].Value
    $munRaw  = $m.Groups[3].Value
    $latOld  = [double]::Parse($m.Groups[4].Value, [System.Globalization.CultureInfo]::InvariantCulture)
    $lonOld  = [double]::Parse($m.Groups[6].Value, [System.Globalization.CultureInfo]::InvariantCulture)
